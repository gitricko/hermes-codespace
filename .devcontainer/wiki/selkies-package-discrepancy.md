...
The PyPI package `selkies==1.6.1` is **NOT** the pixelflux-based selkies described in most specs. It is the **legacy `selkies_gstreamer`** package — an older GStreamer/WebRTC project with:
...
This is a common trap: `pip install selkies` gives you the wrong thing entirely.

## Correct Package

The pixelflux-based `selkies` (v0.0.0.dev0) with:
- Real Selkies web client (React dashboard with sidebar)
...
- Wiki: `codespace-webtop.md` — full architecture and setup
- Skill: `codespace-webtop` — control script that builds the correct wheel and web client
